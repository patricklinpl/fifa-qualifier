...
const getAllGameStats = async (confederation, confGameIds) => {
  ...
  const halfIndex = Math.ceil(confGameIds['id'].length / 2)
  const left = confGameIds['id'].splice(0, halfIndex)
  const right = confGameIds['id'].splice(halfIndex, confGameIds['id'].length)

  left.forEach((gameid) => {
    const scrapeURL = `http://www.espn.com/soccer/matchstats?gameId=${gameid}`
    promisesLeft.push(scrapeGameSummary(scrapeURL, gameid, confSummarys))
  })

  await Promise.all(promisesLeft)

  right.forEach((gameid) => {
    const scrapeURL = `http://www.espn.com/soccer/matchstats?gameId=${gameid}`
    promisesRight.push(scrapeGameSummary(scrapeURL, gameid, confSummarys))
  })

  await Promise.all(promisesRight)

...
const getAllGameStats = async (confederation, confGameIds) => {
  ...
  console.log(`[${confederation}] scraped ${confSummarys.length}`)
...
const scrapeGameSummary = async (scrapeURL, gameid, confSummarys) => {
  try {
    ...
    const team1Score = $('span[data-home-away=home][data-stat=score]').text().replace(/\s/g, '')
    ...
    const team2Score = $('span[data-home-away=away][data-stat=score]').text().replace(/\s/g, '')
    ...
    if (!team1Yellows & !team1Reds & !team2Yellows & !team2Reds) {
      const matchURL = `http://www.espn.com/soccer/match?gameId=${gameid}`
      const testScrapeURL = await testNoSummaryURL(matchURL)
      if (testScrapeURL) {
    ...
    console.log(`[${gameid}] scraped ${scrapeURL}`)
...
/**
 * test scrape url before using
 * @param {string} scrapeURL - url to scrape from
 * @param {boolean}
 */
const testNoSummaryURL = async (url) => {
  try {
    await axios.get(url)
    return true
  } catch (error) {
    console.log(`Error on request for ${url} so skip and just add score`)
    return false
  }
}
